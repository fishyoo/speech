...
void onSentenceBegin(NlsEvent* cbEvent, void* cbParam) {
	...
	cout << "onSentenceBegin: "
		<< "status code: " << cbEvent->getStausCode()  // 获取消息的状态码，成功为0或者20000000，失败时对应失败的错误码
		<< ", task id: " << cbEvent->getTaskId()   // 当前任务的task id，方便定位问题，建议输出
		<< ", index: " << cbEvent->getSentenceIndex()
		<< ", time: " << cbEvent->getSentenceTime()
		<< endl;
 ...
 * 识别单个音频数据
 */
int speechTranscriberFile(const char* token, const char* appkey) {
...
int speechTranscriberMultFile(const char* token, const char* appkey) {
